...
export const HeroCarousel: React.FC = React.memo(() => {
  ...
  // GSAP slide transitions (image cross-fade + text entrance)
  useEffect(() => {
  ...
  return (
    <>
      {/* HeroBlanket is portal-mounted to document.body — position:fixed is viewport-guaranteed */}
      <HeroBlanket heroRef={heroContainerRef} />

      <section
        ref={heroContainerRef}
        className="relative w-full h-screen flex flex-col md:flex-row bg-[#F5FAFD]/40 overflow-hidden"
      >
        {/* Left Carousel Image Section (~58%) */}
        <div
          ref={imageContainerRef}
          className="relative w-full md:w-[58%] h-[50vh] md:h-full overflow-hidden bg-navy-deep flex-shrink-0"
        >
          {slides.map((slide, idx) => (
            <div
              key={slide.id}
              className={`absolute inset-0 slide-img slide-img-${idx} opacity-0 w-full h-full`}
        ...
        {/* Right Info Panel (~42%) */}
        <div className="w-full md:w-[42%] h-auto md:h-full flex flex-col justify-center p-8 sm:p-12 md:p-16 bg-white relative overflow-y-auto">
          {/* Slide dots */}
          ...
          <div ref={textContainerRef} className="space-y-4 max-w-md my-auto">
            ...
            <div className="pt-3">
              ...
              <span className="font-sans text-xl font-bold text-navy-deep">From {slides[activeIndex].price}</span>
            </div>
            <div className="pt-6">
              <Link
                to={slides[activeIndex].link}
                className="inline-block bg-navy-deep text-white font-sans text-xs uppercase tracking-wide font-semibold px-8 py-3.5 rounded-full hover:bg-royal-blue transition-luxury shadow-md"
              >
                SHOP THE COLLECTION →
              </Link>
            </div>
