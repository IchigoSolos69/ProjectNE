...
function wrapWithBrandLayout(title, bodyContent) {
    return `
    <!DOCTYPE html>
    <html>
      <head>
...
exports.emailService = {
    sendWelcomeEmail: async (email, name) => {
        const html = wrapWithBrandLayout('Welcome to Quiet Luxury', `
      <p>Dear ${name},</p>
    ...
    sendPasswordResetEmail: async (email, token, name) => {
        ...
        const html = wrapWithBrandLayout('Reset Your Password', `
      <p>Hello ${name},</p>
    ...
    sendOrderConfirmationEmail: async (email, order, name) => {
        const orderRef = order.id.slice(-8).toUpperCase();
        const html = wrapWithBrandLayout(`Order Confirmed — Reference #${orderRef}`, `
      <p>Dear ${name},</p>
    ...
    sendOrderShippedEmail: async (email, order, name, trackingNumber, carrier) => {
        const orderRef = order.id.slice(-8).toUpperCase();
        const html = wrapWithBrandLayout(`Your Bedding is on the Way — Order #${orderRef}`, `
      <p>Dear ${name},</p>
    ...
    sendOrderDeliveredEmail: async (email, order, name) => {
        const orderRef = order.id.slice(-8).toUpperCase();
        const html = wrapWithBrandLayout('Your Linens Have Arrived', `
      <p>Dear ${name},</p>
    ...
    sendOrderCancelledEmail: async (email, order, name) => {
        const orderRef = order.id.slice(-8).toUpperCase();
        const html = wrapWithBrandLayout(`Order Status Update — Order #${orderRef}`, `
      <p>Dear ${name},</p>
