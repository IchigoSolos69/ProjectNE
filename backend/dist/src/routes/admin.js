...
router.use(requireAuth_1.requireAdmin);
...
router.post('/products', async (req, res) => {
    try {
        const { name, description, images = [], isTrending = false, isActive = true, categoryId, variants = [], // array of { size, color, sku, price, stock, discountPrice }
         } = req.body;
        ...
        let slugExists = await prisma_1.prisma.product.findUnique({ where: { slug } });
        let counter = 1;
        while (slugExists) {
            slug = `${slugify(name)}-${counter}`;
            slugExists = await prisma_1.prisma.product.findUnique({ where: { slug } });
            counter++;
        }
        // Check unique SKUs
...
router.patch('/products/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { name, description, images, isTrending, isActive, categoryId, variants, } = req.body;
...
router.patch('/orders/:id/status', async (req, res) => {
    try {
        ...
        if (status === 'CONFIRMED') {
            emails_1.emailService.sendOrderConfirmationEmail(email, updatedOrder, name);
        }
        else if (status === 'SHIPPED') {
            emails_1.emailService.sendOrderShippedEmail(email, updatedOrder, name, trackingNumber || '', carrier || '');
        }
        else if (status === 'DELIVERED') {
            emails_1.emailService.sendOrderDeliveredEmail(email, updatedOrder, name);
        }
        else if (status === 'CANCELLED' || status === 'REFUNDED') {
            emails_1.emailService.sendOrderCancelledEmail(email, updatedOrder, name);
        }
...
router.post('/coupons', async (req, res) => {
    try {
        const { code, type, value, minOrderValue, maxDiscountAmount, usageLimit, usageLimitPerUser = 1, validFrom, validUntil, isActive = true, } = req.body;
        if (!code || !type || value === undefined || !validFrom || !validUntil) {
            return res.status(400).json({ error: 'Missing required coupon fields.' });
        }
        const codeUpper = code.toUpperCase().trim();
        const existing = await prisma_1.prisma.coupon.findUnique({ where: { code: codeUpper } });
        if (existing) {
...
router.patch('/coupons/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { code, type, value, minOrderValue, maxDiscountAmount, usageLimit, usageLimitPerUser, validFrom, validUntil, isActive, } = req.body;
        ...
        if (code !== undefined) {
            ...
            if (codeUpper !== existing.code) {
                const clash = await prisma_1.prisma.coupon.findUnique({ where: { code: codeUpper } });
                if (clash)
                    return res.status(400).json({ error: `Coupon code "${codeUpper}" is already in use.` });
            }
        ...
        if (minOrderValue !== undefined)
            updateData.minOrderValue = minOrderValue ? Number(minOrderValue) : null;
        ...
        if (usageLimitPerUser !== undefined)
            updateData.usageLimitPerUser = usageLimitPerUser ? parseInt(usageLimitPerUser, 10) : null;
...
router.post('/upload', async (req, res) => {
    try {
        ...
        const signatureDetails = (0, cloudinary_1.generateUploadSignature)(params);
