...
function sendAuthToken(res, userId, userPayload) {
...
router.post('/register', async (req, res) => {
    try {
        ...
        emails_1.emailService.sendWelcomeEmail(user.email, user.name);
...
router.post('/google', async (req, res) => {
    try {
        ...
        // Verify Google Token
        const googleProfile = await (0, google_1.verifyGoogleToken)(idToken);
        // Find or create user
...
router.post('/forgot-password', async (req, res) => {
    try {
        ...
        emails_1.emailService.sendPasswordResetEmail(user.email, rawToken, user.name);
        return res.status(200).json({
            message: 'If that email address is registered, we have sent instructions to reset your password.',
        });
    }
